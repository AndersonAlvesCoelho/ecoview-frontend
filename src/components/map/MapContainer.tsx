      ...
      >
        <defs>
          <pattern id="grid" width="60" height="60" patternUnits="userSpaceOnUse">
            <path d="M60 0 L0 0 L0 60" fill="none" stroke="#c8d3c8" strokeOpacity="0.35" strokeWidth="0.6" />
          </pattern>
          <pattern id="grid-major" width="240" height="240" patternUnits="userSpaceOnUse">
            <path d="M240 0 L0 0 L0 240" fill="none" stroke="#a8b7a8" strokeOpacity="0.35" strokeWidth="0.8" />
          </pattern>
        ...
        <g transform="translate(0,0)">
          ...
          {activeLayers.some((l) => l.id === "l-biomas") && (
            <g opacity={activeLayers.find((l) => l.id === "l-biomas")?.opacity ?? 0.55}>
              <path d="M420 110 C 520 90, 640 100, 740 140 C 820 172, 870 210, 890 260 L 640 380 L 460 320 L 380 220 Z" fill="#3b8b3b" fillOpacity="0.55" />
              <path d="M380 220 L 460 320 L 640 380 L 700 480 L 540 540 L 420 520 L 340 430 Z" fill="#c4b25a" fillOpacity="0.65" />
              <path d="M700 480 L 890 260 L 930 340 L 900 460 L 830 540 Z" fill="#e2a34a" fillOpacity="0.55" />
              <path d="M420 520 L 540 540 L 700 480 L 830 540 L 780 640 L 620 700 L 460 660 Z" fill="#5aa869" fillOpacity="0.55" />
              <path d="M460 660 L 620 700 L 640 720 L 540 745 L 440 730 L 400 700 Z" fill="#8fb46d" fillOpacity="0.6" />
              <path d="M340 430 L 420 520 L 460 660 L 400 700 L 340 660 L 300 580 Z" fill="#c78a5d" fillOpacity="0.5" />
            </g>
          )}
          ...
          {activeLayers.some((l) => l.id === "l-uc") && (
            <g opacity={activeLayers.find((l) => l.id === "l-uc")?.opacity ?? 0.7}>
              <ellipse cx="560" cy="260" rx="34" ry="22" fill="#008a4b" fillOpacity="0.35" stroke="#008a4b" strokeWidth="1.2" />
              <ellipse cx="480" cy="380" rx="28" ry="18" fill="#008a4b" fillOpacity="0.35" stroke="#008a4b" strokeWidth="1.2" />
              <ellipse cx="700" cy="330" rx="24" ry="20" fill="#008a4b" fillOpacity="0.35" stroke="#008a4b" strokeWidth="1.2" />
              <ellipse cx="600" cy="580" rx="30" ry="18" fill="#008a4b" fillOpacity="0.35" stroke="#008a4b" strokeWidth="1.2" />
            </g>
          )}

          {/* Estados outline */}
          {activeLayers.some((l) => l.id === "l-estados") && (
            <g opacity={activeLayers.find((l) => l.id === "l-estados")?.opacity ?? 0.9} fill="none" stroke="#006633" strokeWidth="1.4">
              ...
              <path d="M380 220 L 460 320 L 640 380 L 700 480 L 540 540 L 420 520 L 340 430 Z" strokeOpacity="0.5" />
          ...
          {activeLayers.some((l) => l.id === "l-ti") && (
            <g opacity={activeLayers.find((l) => l.id === "l-ti")?.opacity ?? 0.7}>
              {[[520, 300], [590, 340], [500, 420], [640, 260], [720, 400]].map(([x, y], i) => (
                <path key={i} d={`M${x} ${y} l 24 6 l -6 22 l -26 -4 z`} fill="#a86b2a" fillOpacity="0.35" stroke="#a86b2a" strokeWidth="1" />
              ))}
            </g>
          )}
          ...
          {activeLayers.some((l) => l.id === "l-municipios") && (
            <g opacity={activeLayers.find((l) => l.id === "l-municipios")?.opacity ?? 0.6} fill="none" stroke="#339966" strokeWidth="0.4">
              {Array.from({ length: 14 }).map((_, i) => (
                <line key={`h${i}`} x1={280} x2={920} y1={140 + i * 42} y2={140 + i * 42} strokeOpacity="0.6" />
              ))}
              {Array.from({ length: 16 }).map((_, i) => (
                <line key={`v${i}`} x1={280 + i * 42} x2={280 + i * 42} y1={140} y2={720} strokeOpacity="0.6" />
              ))}
            </g>
          )}
          ...
          {activeLayers.some((l) => l.id === "l-prodes") && (
            <g opacity={activeLayers.find((l) => l.id === "l-prodes")?.opacity ?? 0.75}>
              {[[520, 340, 30, 18], [560, 380, 22, 14], [610, 350, 20, 12], [480, 320, 18, 10], [640, 300, 16, 9]].map(([x, y, w, h], i) => (
                <rect key={i} x={x} y={y} width={w} height={h} fill="#7f1d1d" fillOpacity="0.55" transform={`rotate(${(i * 17) % 40} ${x + w / 2} ${y + h / 2})`} />
              ))}
            </g>
          )}
          ...
          {activeLayers.some((l) => l.id === "l-hidro") && (
            <g opacity={activeLayers.find((l) => l.id === "l-hidro")?.opacity ?? 0.7} fill="none" stroke="#0369a1" strokeLinecap="round">
          ...
          {activeLayers.some((l) => l.id === "l-uso-solo") && (
            <g opacity={activeLayers.find((l) => l.id === "l-uso-solo")?.opacity ?? 0.7}>
              {Array.from({ length: 40 }).map((_, i) => {
                ...
                return <rect key={i} x={cx} y={cy} width={70} height={70} fill={c} fillOpacity="0.35" />;
              })}
            </g>
          )}
          ...
          <g>
            ...
            <circle cx="700" cy="200" r="6" fill="#006633" stroke="#ffffff" strokeWidth="2" onClick={onFeatureClick} className="cursor-pointer" />
          </g>
        </g>
      </svg>
